...
export default async function SpotifyCallbackPage({ searchParams }: SpotifyCallbackPageProps) {
  ...
  return (
    <main className="mx-auto min-h-screen w-full max-w-2xl px-5 pb-16 pt-16 sm:px-8">
      <h1 className="mb-2 text-2xl font-bold text-navy">Spotify Callback</h1>
      ...
      <div className="section-card p-4">
        {error ? (
          <p className="text-sm text-red-600">Spotify returned an error: {error}</p>
        ) : code ? (
          <>
            <p className="mb-2 text-sm text-zinc-700">Authorization code:</p>
          ...
          <p className="text-sm text-zinc-600">No code found in query params.</p>
        )}
      </div>
    </main>
  );
